...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    ...
    const filteredApps = appsList.filter(
      eachObject =>
        eachObject.category === activeTabId &&
        eachObject.appName.toLowerCase().includes(searchText.toLowerCase()),
    )
    return filteredApps
  }
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="apps-container">
          {filteredApps.map(eachAppObject => (
            <AppItem key={eachAppObject.appId} appDetails={eachAppObject} />
          ))}
        </ul>
      </div>
    )
